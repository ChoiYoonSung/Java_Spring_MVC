					...
					<div class="col-sm-12">
						...
						<c:forEach var="notice" items="${noticeList }" >
							<tr style="cursor:pointer;">
								<td style="width:15%">${notice.nno }</td>
								<td style="width:45%"><a href="javascript:OpenWindow('detail.do?nno=${notice.nno }','공지사항 상세보기','800','900')">${notice.title }</a></td>
								<td style="width:20%">${notice.writer }</td>
								<td style="width:15%"><fmt:formatDate value="${notice.regdate }" pattern="yyyy년 MM월 dd일"/> </td>
								<td style="width:15%">${notice.viewcnt }</td>
							</tr>
						</c:forEach>
						</table>
					</div>
